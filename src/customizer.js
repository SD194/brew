/**
 * BrewSync — Item Customizer
 * Bottom sheet for customizing menu items (size, milk, sugar, notes).
 */
import * as Cart from './cart.js';
import { closeAllSheets, openSheet } from './ui.js';
...
export function openCustomizer(item, categorySlug) {
  currentItem = item;
  ...
  openSheet('customizerSheet');
  setupCustomizerListeners(item, options);
}

function setupCustomizerListeners(item, options) {
  const content = document.getElementById('customizerContent');
  ...
  document.getElementById('custAddBtn').addEventListener('click', () => {
    ...
    Cart.addItem(item.id, item.name, totalPrice, item.id);

    // Store notes in cart item
    const cart = Cart.getCart();
    if (cart[item.id]) {
      cart[item.id].notes = notes;
      cart[item.id].price = totalPrice;
    }
